...
export default function JobDetail() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0c0c0e] text-gray-300">
      <Navbar />
      <div className="text-center py-32 text-gray-600 font-mono text-sm animate-pulse">
        Loading job...
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-gray-300">
      ...
      <div className="px-8 pt-6">
        <button onClick={() => navigate(-1)}
          className="text-xs text-gray-600 hover:text-gray-300 transition-colors flex items-center gap-1.5 font-mono">
          ← back to jobs
        </button>
      </div>

      <div className="px-8 py-8 max-w-6xl mx-auto grid grid-cols-3 gap-10">
        ...
        <div className="col-span-2 flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-5">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-xl flex items-center justify-center text-base font-bold font-mono flex-shrink-0 bg-blue-950 text-blue-400">
            ...
            <div className="flex flex-wrap gap-3 text-xs font-mono">
              {[
                ...
                { icon: "⊕", val: `${job?.openings} opening${job?.openings > 1 ? "s" : ""}` },
              ].map((m) => m.val && (
          ...
          <div className="h-px bg-[#1a1a1e]" />
          <div className="flex flex-col gap-4">
            <h2 className="text-xs uppercase tracking-widest text-gray-600 font-mono">
              About {job?.postedBy?.companyName || job?.postedBy?.name}
            </h2>
            <p className="text-sm text-gray-400 leading-7 font-light">
              {job?.postedBy?.companyName
                ? `${job.postedBy.companyName} is actively hiring for this position. Connect with them to learn more.`
                : "The recruiter is actively looking for candidates for this role."}
            </p>
          </div>
        </div>
        ...
        <div className="col-span-1">
          <div className="sticky top-6 flex flex-col gap-4">
            ...
            <div className="bg-[#0f0f11] border border-[#1e1e22] rounded-2xl p-5 flex flex-col gap-4">
              ...
              {error && (
                <p className="text-[10px] text-red-400 bg-red-950 border border-red-900 rounded-lg px-3 py-2">
                  {error}
                </p>
              )}
            ...
            <button onClick={() => navigate(-1)}
              className="text-xs text-gray-600 hover:text-gray-300 transition-colors border border-[#1e1e22] rounded-xl py-2.5 bg-[#0f0f11]">
              ← Back to listings
            </button>
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-[#0f0f11] border border-[#2a2a2e] rounded-2xl p-7 w-full max-w-md flex flex-col gap-5">
            <div>
            ...
            <div className="bg-[#141416] border border-[#1e1e22] rounded-xl p-4 flex flex-col gap-2 text-xs">
              ...
              <div className="flex items-center gap-2 text-gray-400"><span className="text-blue-500">✦</span> Your profile (name, skills, education)</div>
              <div className="flex items-center gap-2 text-gray-400"><span className="text-blue-500">✦</span> Your uploaded resume (PDF)</div>
