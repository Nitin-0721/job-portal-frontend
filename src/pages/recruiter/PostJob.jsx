...
export default function PostJob() {
  ...
  const inputClass = "bg-[#0f0f11] border border-[#1e1e22] rounded-xl px-4 py-3 text-sm text-gray-200 outline-none focus:border-blue-500 transition-colors placeholder-gray-700 w-full";
  const selectClass = "bg-[#0f0f11] border border-[#1e1e22] rounded-xl px-4 py-3 text-sm text-gray-200 outline-none focus:border-blue-500 transition-colors w-full";
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-gray-300">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-8 py-8 sm:py-10">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-gray-100">Post a Job</h1>
          <p className="text-sm text-gray-500 mt-2 font-light">Fill in the details to publish your listing.</p>
        </div>

        {/* STEP INDICATOR */}
        <div className="flex items-center gap-0 mb-8 sm:mb-10 overflow-x-auto pb-2">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center flex-1 min-w-0">
              <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                ...
                <span className={`text-[9px] sm:text-[10px] font-mono whitespace-nowrap ${i === step ? "text-blue-400" : "text-gray-700"}`}>
                  {s}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-px mx-1 sm:mx-2 mb-4 ${i < step ? "bg-blue-500" : "bg-[#1e1e22]"}`} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="title" value={form.title} onChange={handleChange}
                placeholder="e.g. React Frontend Developer" className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              {[
                ...
                <div key={f.name} className="flex flex-col gap-1.5">
                  ...
                  <input name={f.name} value={form[f.name]} onChange={handleChange}
                    type={f.type || "text"} placeholder={f.placeholder} className={inputClass} />
                </div>
              ))}
              <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => addSkill(skillInput)}
                  className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-4 sm:px-5 rounded-xl transition-colors flex-shrink-0">
                  Add
                </button>
              </div>
        ...
        {step === 3 && (
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-[#0f0f11] border border-[#1e1e22] rounded-2xl p-4 sm:p-6 flex flex-col gap-4 sm:gap-5">
              <div>
                <h2 className="text-lg sm:text-xl font-semibold text-gray-100 tracking-tight">{form.title || "—"}</h2>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-[#1a1a1e]">
          <button onClick={() => step > 0 ? setStep(step - 1) : navigate("/dashboard")}
            className="text-xs text-gray-500 border border-[#2a2a2e] rounded-xl px-4 sm:px-5 py-2.5 hover:text-gray-300 transition-colors">
            ← {step === 0 ? "Cancel" : "Back"}
          </button>
          {step < STEPS.length - 1 ? (
            <button onClick={() => isStepValid() && setStep(step + 1)}
              className={`text-xs font-medium px-5 sm:px-6 py-2.5 rounded-xl transition-colors ${
            ...
            <button onClick={handlePost} disabled={posting || posted}
              className="text-xs font-medium bg-blue-500 hover:bg-blue-600 text-white px-5 sm:px-6 py-2.5 rounded-xl transition-colors disabled:opacity-50">
