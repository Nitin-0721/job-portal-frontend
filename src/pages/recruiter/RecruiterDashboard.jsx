      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        ...
        <div className="flex justify-between items-start mb-10">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-gray-100">Hey, {name} 👋</h1>
        ...
        <div className="grid grid-cols-4 gap-3 mb-10">
          {[
            { label: "Jobs Posted", val: stats.totalJobs, color: "text-gray-100", sub: "total" },
            { label: "Active Listings", val: stats.activeJobs, color: "text-blue-400", sub: "live now" },
            { label: "Total Applicants", val: stats.totalApplicants, color: "text-yellow-400", sub: "across all jobs" },
            { label: "Shortlisted", val: stats.shortlisted, color: "text-green-400", sub: "this week" },
          ].map((s) => (
            <div key={s.label} className="bg-[#0f0f11] border border-[#1e1e22] rounded-xl p-5">
              <p className={`text-3xl font-mono font-semibold ${s.color}`}>{s.val}</p>
              <p className="text-xs text-gray-400 mt-1">{s.label}</p>
              <p className="text-[10px] text-gray-700 mt-0.5 font-mono">{s.sub}</p>
            </div>
          ))}
        </div>
            ...
            <div className="col-span-2 flex flex-col gap-4">
                ...
                <div className="flex gap-1 bg-[#0f0f11] border border-[#1e1e22] rounded-lg p-1">
                  {["all", "active", "closed"].map((f) => (
                    <button key={f} onClick={() => setFilter(f)}
                      className={`text-[10px] font-mono px-3 py-1.5 rounded-md capitalize transition-all ${
              ...
              {filteredJobs.length === 0 ? (
                ...
                filteredJobs.map((job) => {
                  ...
                  const jobApplicants = applicants.filter((a) => a.job?._id === job._id || a.job === job._id);
                  return (
                    <div key={job._id}
                      className={`bg-[#0f0f11] border border-[#1e1e22] rounded-xl p-5 ${job.status === "closed" ? "opacity-50" : ""}`}>
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-xs text-gray-600 font-mono">{job.type} · {job.mode} · {job.salaryMin}{job.salaryMax ? ` – ${job.salaryMax}` : ""}</p>
                        </div>
