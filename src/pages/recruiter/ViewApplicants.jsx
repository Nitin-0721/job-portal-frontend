...
export default function ViewApplicants() {
  ...
  const DetailPanel = () => (
    <div className="bg-[#0f0f11] border border-[#1e1e22] rounded-2xl p-5 sm:p-6 flex flex-col gap-5 sm:gap-6">
      {/* Mobile back button */}
      <button onClick={() => setShowDetail(false)}
        className="lg:hidden text-xs text-gray-500 hover:text-gray-300 flex items-center gap-1 font-mono">
        ← Back to list
      </button>

      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-blue-950 border-2 border-blue-900 flex items-center justify-center text-base sm:text-lg font-mono font-bold text-blue-400">
            {selected.applicant?.name?.split(" ").map((n) => n[0]).join("") || "?"}
          </div>
          <div>
            <h2 className="text-sm sm:text-base font-semibold text-gray-100">{selected.applicant?.name}</h2>
        ...
        <span className={`text-[10px] font-mono border rounded-full px-2 sm:px-3 py-1 flex-shrink-0 ml-2 ${STATUS_CONFIG[selected.status].bg} ${STATUS_CONFIG[selected.status].color}`}>
          ...
          {["shortlisted", "pending", "rejected"].map((s) => (
            <button key={s} onClick={() => updateStatus(selected._id, s)}
              ...
              {selected.status === s
                ...
                : s === "shortlisted" ? "Shortlist" : s === "pending" ? "Review" : "Reject"}
            </button>
          ))}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-gray-300">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-10">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-gray-100">Applicants</h1>
          ...
          <>
            ...
            {!showDetail && (
              <div className="lg:hidden flex flex-col gap-3">
                {/* Filter tabs */}
                <div className="flex gap-1 bg-[#0f0f11] border border-[#1e1e22] rounded-xl p-1 overflow-x-auto">
                  {["all", "pending", "shortlisted", "rejected"].map((f) => (
                    <button key={f} onClick={() => setFilter(f)}
                      className={`flex-1 text-[10px] font-mono py-1.5 rounded-lg capitalize transition-all whitespace-nowrap ${
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <div key={a._id} onClick={() => handleSelectApplicant(a)}
                      className="bg-[#0f0f11] border border-[#1e1e22] rounded-xl p-4 cursor-pointer hover:border-[#333] transition-all">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-9 h-9 rounded-full bg-blue-950 border border-blue-900 flex items-center justify-center text-[10px] font-mono font-bold text-blue-400">
                            {a.applicant?.name?.split(" ").map((n) => n[0]).join("") || "?"}
                          </div>
                          <div>
                            <p className="text-sm font-medium text-gray-200">{a.applicant?.name}</p>
                            <p className="text-xs text-gray-600">{a.applicant?.degree || "Student"}</p>
                          </div>
                        </div>
                      ...
                      <p className="text-[10px] text-gray-500 mt-1 font-mono">Tap to view profile →</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="hidden lg:grid grid-cols-3 gap-6">
              ...
              <div className="col-span-1 flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  {filtered.length === 0 ? (
                    <p className="text-xs text-gray-700 text-center py-8">No applicants</p>
                  ) : filtered.map((a) => {
                    const st = STATUS_CONFIG[a.status];
                    return (
