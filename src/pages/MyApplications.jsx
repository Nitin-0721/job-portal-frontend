    ...
    <div className="min-h-screen bg-[#0c0c0e] text-gray-300">
      ...
      <div className="max-w-4xl mx-auto px-8 py-10">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-gray-100">Applications</h1>
          <p className="text-sm text-gray-500 mt-2 font-light">
            Track every job you've applied to — all in one place.
          </p>
        </div>
        ...
        {/* FILTER TABS */}
        <div className="flex gap-1 mb-6 bg-[#0f0f11] border border-[#1e1e22] rounded-xl p-1 w-fit">
          {FILTERS.map((f) => (
            <button key={f} onClick={() => setActiveFilter(f)}
              className={`text-xs px-4 py-2 rounded-lg transition-all font-mono ${
        ...
        {loading ? (
          <div className="text-center py-20 text-gray-600">
            <p className="text-sm font-mono animate-pulse">Loading applications...</p>
          </div>
        ) : (
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20 text-gray-600">
                ...
                <button onClick={() => navigate("/")}
                  className="mt-4 text-xs text-blue-400 underline underline-offset-4">
                  Browse jobs →
                </button>
              </div>
            ) : (
              filtered.map((app) => {
                ...
                return (
                  <div key={app._id}
                    ...
                    <div className="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-[#111] transition-colors"
                      onClick={() => setExpanded(isExpanded ? null : app._id)}>
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold font-mono bg-blue-950 text-blue-400 flex-shrink-0">
                        {job?.title?.slice(0, 2).toUpperCase() || "JB"}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-200 truncate">{job?.title}</p>
                        <p className="text-xs text-gray-500">
                          {job?.postedBy?.companyName || job?.postedBy?.name} · {job?.location} · {job?.type}
                        </p>
                      </div>
                      <div className={`flex items-center gap-1.5 text-[10px] font-mono border rounded-full px-3 py-1 flex-shrink-0 ${status.bg} ${status.color}`}>
                    ...
                    {isExpanded && (
                      <div className="border-t border-[#1a1a1e] px-5 py-4 flex flex-col gap-4">
